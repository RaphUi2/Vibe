...
const Vibeos: React.FC<{ user: User }> = ({ user }) => {
  ...
  return (
    <div className="h-full w-full overflow-y-scroll snap-y snap-mandatory scrollbar-hide bg-black relative">
      <div className="absolute top-10 right-6 z-[100]">
         <button 
           onClick={() => window.dispatchEvent(new CustomEvent('vibeOpenPostModal'))}
           className="w-14 h-14 bg-white text-black rounded-2xl flex items-center justify-center shadow-4xl hover:scale-105 active:scale-95 transition-all"
         >
